...
datas <- list(model_1 = list(
  N = total_days, y = customers$customers,
  T = c(change_point_1, change_point_2)
))
datas$model_2 <- datas$model_1
datas$model_2$d <- c(transition_period_1, transition_period_2)

draws <- purrr::map2(models, datas, ~ sampling(.x, data = .y, refresh = 0))
```

```{r bridge_samples, cache=TRUE, dependson='mcmc_draws'}
bridge_samples <- purrr::map(draws, bridge_sampler, silent = TRUE)
marginal_likelihoods <- purrr::map_dbl(bridge_samples, ~ .x$logml)
```
...
```{r model_comparison}
data_frame(model = names(bridge_samples), post_prob = post_prob(marginal_likelihoods)) %>%
